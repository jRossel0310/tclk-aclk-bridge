...
// wrapper has NO AXI interface: the BD wires only clk_os (the 80 MHz cell clock
// from the clk_wiz MMCM) and rstn (from the proc_sys_reset). It drives the
...
// 80 MHz, so divide it down).

`timescale 1ns / 1ps

module aclk_gen_bd_top (
    input  wire clk_os,            // 80 MHz cell clock (from the BD clk_wiz)
    ...
    // Divide clk_os down so a Pmod level translator can pass it: clkos_dbg ~78 kHz
    // if clk_os is alive.
