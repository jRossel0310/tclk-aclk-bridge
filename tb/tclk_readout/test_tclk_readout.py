...
# Register byte offsets (16-byte spacing — see aclk_readout_axi.sv). EVENT returns {FLAGS, EVENT}.
...
async def reset_dut(dut):
    ...
    await ClockCycles(dut.clk_40m, 5)
    # Post-reset quiescent settle, scaled to the oversample ratio. The serdec runs on
    # clk_80m with a TCLK_del shift register ~1.5*OSR deep. After reset the shift
    # register is flushed; this settle window gives it time to stabilize before the
    # driver's warmup carrier begins, ensuring the decode FSM anchors correctly on
    # the first cell. The empirical factor-2 margin in `2*(SAMPLES_PER_CELL-8)` accounts
    # for OSR scaling; it is zero at the default OSR=8, keeping regression bit-identical.
