...
This is the PL block Phase B synthesizes for the KR260. The line input is driven
by the shared Manchester model (tb/manchester_tx_model.py) and events are read
over AXI with the shared BFM (tb/axi_lite_bfm.py).
...
BUILD    = PROJ_DIR / "sim_build" / "aclk_lite_readout"

sys.path.insert(0, str(TB_DIR))
sys.path.insert(0, str(TB_DIR.parent))    # shared tb/manchester_tx_model.py + tb/axi_lite_bfm.py
...
def test_aclk_lite_readout():
    ...
    runner.build(
        sources=[
            ...
            RTL_DIR / "aclk_lite" / "aclk_lite_readout_top.sv",
        ],
    ...
    runner.test(
        hdl_toplevel="aclk_lite_readout_top",
        test_module="test_aclk_lite_readout",
        build_dir=BUILD,
        waves=True,
    )
