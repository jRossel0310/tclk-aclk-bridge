"""Cocotb 2.0 Python runner for the button_parser testbench.

Switch simulators by changing SIM below, or from the shell:
    $env:SIM = "verilator"      # PowerShell
    export SIM=verilator        # bash
"""

import os
import sys
from pathlib import Path

from cocotb_tools.runner import get_runner

# ===== flip this ONE variable to change simulator ========================
SIM = os.getenv("SIM", "icarus")        # "icarus" (default) or "verilator"
# =========================================================================

TB_DIR   = Path(__file__).resolve().parent
PROJ_DIR = TB_DIR.parents[1]
RTL_DIR  = PROJ_DIR / "rtl"
BUILD    = PROJ_DIR / "sim_build" / "button_parser"

sys.path.insert(0, str(TB_DIR))

# Best-effort: honor OSS_CAD_SUITE if set; otherwise rely on the tools already
# being on PATH (the sim.sh / sim.ps1 wrappers put them there for you).
_oss = os.getenv("OSS_CAD_SUITE")
if _oss and (Path(_oss) / "bin").is_dir():
    os.environ["PATH"] = str(Path(_oss) / "bin") + os.pathsep + os.environ.get("PATH", "")


def test_button_parser():
    runner = get_runner(SIM)
    build_args = ["--trace-fst", "--trace-structs"] if SIM == "verilator" else []
    runner.build(
        # button_parser instantiates these submodules, so they all compile in.
        sources=[
            RTL_DIR / "button_parser.sv",
            RTL_DIR / "synchronizer.sv",
            RTL_DIR / "debouncer.sv",
            RTL_DIR / "edge_detector.sv",
        ],
        hdl_toplevel="button_parser",
        build_dir=BUILD,
        build_args=build_args,
        # Tiny debounce window so the test runs fast. Keep in sync with the
        # constants at the top of test_button_parser.py.
        parameters={"SAMPLE_CNT_MAX": 4, "PULSE_CNT_MAX": 4},
        timescale=("1ns", "1ps"),
        waves=True,
        always=True,
    )
    runner.test(
        hdl_toplevel="button_parser",
        test_module="test_button_parser",
        build_dir=BUILD,
        waves=True,
    )


if __name__ == "__main__":
    test_button_parser()
